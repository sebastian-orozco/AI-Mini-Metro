...
class Mediator:
    ...
    def render(self, screen: pygame.surface.Surface) -> None:
        ...
        for station in self.stations:
            station.draw(screen)
            station.render_visual_timer(screen, max_capacity_time_limit_ms=time_limit)
    ...
    def check_whether_not_crossed(self, station: Station, station_list=[]):
        ...
        return whether_not_crossed
    def check_whether_loop(self, station: Station, station_list=[]):
        if station_list is None:
            station_list = self.path_being_created.stations
        assert station_list is not None
    ...
    def add_station_to_path(self, station: Station) -> None:
        ...
        if len(self.path_being_created.stations) > 1:
            # loop
            if self.check_whether_loop(station, self.path_being_created.stations):
            ...
            # non-loop
                ...
                # crossed
                if self.check_whether_not_crossed(station, self.path_being_created.stations):
    ...
    def increment_time(self, dt_ms: int) -> None:
        ...
        # check stations for max-capacity violations
        max_capacity_time_limit_ms = time_limit  # 45 seconds
        for station in self.stations:
            station.increment_time_at_max_capacity(dt_ms)
            if station.time_at_max_capacity >= max_capacity_time_limit_ms:
                # self.trigger_game_over()
                self.score = 0
                # print(f"Warning: Station {station.id} has been at max capacity for 45 seconds!")
    ...
    def find_shared_path(self, station_a: Station, station_b: Station) -> Path | None:
        ...
        for path in self.paths:
            ...
            if (station_a in stations) and (station_b in stations):
                dist = abs(stations.index(station_a)-stations.index(station_b))
